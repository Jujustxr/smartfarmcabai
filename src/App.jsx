...
const App = () => {
  ...
  if (!user) {
    return mode === "login" ? (
      <Login onLogin={() => setUser(auth.currentUser)} switchMode={() => setMode("register")} />
    ) : (
      <Register onRegister={() => setUser(auth.currentUser)} switchMode={() => setMode("login")} />
    );
  }
  ...
  return (
    <div
      className="min-h-screen transition-colors duration-500 bg-pink-200 dark:bg-gray-900"
    >
      <Navbar
